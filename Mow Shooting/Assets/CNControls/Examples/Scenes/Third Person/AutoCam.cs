...
namespace UnityStandardAssets.Cameras
{
    ...
    public class AutoCam : PivotBasedCameraRig
    {
        ...
        protected override void FollowTarget(float deltaTime)
        {
			...
			if (!(deltaTime > 0) || m_Target[PlayerPrefs.GetInt("player")] == null)
            {
                return;
            }

            // initialise some vars, we'll be modifying these in a moment
			var targetForward = m_Target[PlayerPrefs.GetInt("player")].forward;
			var targetUp = m_Target[PlayerPrefs.GetInt("player")].up;
			...
			transform.position = Vector3.Lerp(transform.position, m_Target[PlayerPrefs.GetInt("player")].position, deltaTime*m_MoveSpeed);
